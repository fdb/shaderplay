...
// Based on "Menger Journey" by Syntopia.
...
float DE(in vec3 z)
{
    ...
    for (int n = 0; n < Iterations; n++) {
        //z.xy = rotate(z.xy,4.0+2.0*cos( u_time/8.0));
        z = abs(z);
        //z = length(z);
...
float rand(vec2 co){
    return fract(cos(dot(co,vec2(4.898,7.23))) * 2.631);
}

vec4 rayMarch(in vec3 from, in vec3 dir, in vec2 fragCoord) {
    // Add some noise to prevent banding
    float totalDistance = Jitter*rand(fragCoord.xy+vec2(u_time));
    vec3 dir2 = dir;
    float distance;
    int steps = 0;
    vec3 pos;
    for (int i=0; i < MaxSteps; i++) {
        ...
        dir.zy = rotate(dir2.zy,totalDistance*cos( u_time/4.0)*NonLinearPerspective);
...
void main() {
    ...
    vec3 camPos = 0.5*u_time*vec3(1.0,0.0,0.0);
    vec3 target = camPos + vec3(1.0,0.0*cos(u_time),0.0*sin(0.4*u_time));
    ...
    vec2 coord =-1.0+2.0*gl_FragCoord.xy/u_resolution.xy;
    coord.x *= u_resolution.x/u_resolution.y;

    // Get direction for this pixel
    vec3 rayDir = normalize(camDir + (coord.x*camRight + coord.y*camUp)*FieldOfView);

    out_color = rayMarch(camPos, rayDir, gl_FragCoord.xy );
}
